  ...
  return (
    <div className="min-h-screen bg-slateui-background">
      ...
      <main id="main-content" className="mx-auto max-w-7xl px-4 py-8 sm:px-6 sm:py-12 lg:px-8">
        <section className="relative mb-10 overflow-hidden rounded-2xl bg-primary-800 px-6 py-10 text-white shadow-md sm:px-8 sm:py-12">
          <Badge tone="brand">{PKM_IDENTITY.tagline}</Badge>
          <h1 className="mt-5 text-3xl font-black tracking-tight sm:text-5xl">{PKM_IDENTITY.name}</h1>
              ...
              <ol className="space-y-3.5 text-sm leading-7 text-slateui-secondary">
                {PKM_MISSION.map((item, index) => (
                  <li key={item} className="flex gap-3">
                    <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-secondary-100 text-xs font-bold text-slateui-text ring-1 ring-secondary-300">
                      {index + 1}
                    </span>
                    <span>{item}</span>
                  </li>
            ...
            <Card>
              ...
              <ol className="space-y-3.5 text-sm leading-7 text-slateui-secondary">
                {PKM_GOALS.map((item, index) => (
                  <li key={item} className="flex gap-3">
                    <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-primary-50 text-xs font-bold text-primary-800 ring-1 ring-primary-200">
                      {index + 1}
                    </span>
                    <span>{item}</span>
                  </li>
