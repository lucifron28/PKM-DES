...
export default async function SubjectListPage() {
  ...
  const [programsResult, offeringsResult] = await Promise.all([
    supabase
      .from("programs")
      .select("id, name, code")
  ...
  if (programsResult.error || offeringsResult.error) {
    console.error("subject_list:load_failed", {
      programsError: programsResult.error,
      offeringsError: offeringsResult.error
    });
    return (
      <EmptyState
  ...
  const studentProgramCode = studentProgram?.code ?? "BSAIS";
  const studentProgramName = studentProgram?.name ?? "Accounting Information System";
  ...
  return (
    <SubjectReferenceBrowser
      ...
      curriculumSubjects={AIS_SUBJECTS}
    />
  );
}
