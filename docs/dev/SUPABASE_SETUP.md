...
- `verify_health_requirement_with_signature(...)` performs Nurse/program authorization, exact applicability evaluation, server-validated acknowledgment and short administrative note handling, Health Record status verification, Nurse signature insertion, clearance update, and audit insertion in one transaction. The forward-only `20260815000000_digital_health_record_verification.sql` migration removes the older five-argument endpoint and revokes direct authenticated writes to `student_requirements`.
- `reject_health_requirement(...)` is the Nurse-only no-signature rejection path. It stores only the optional short administrative note, writes an audit event, and preserves any historical invalidated signature rows.
...
- `list_nurse_health_requirements()` and `get_nurse_health_requirement(...)` expose only the applicable Nurse status worklist and signature metadata; no clinical fields are returned. Queue status filters are Pending, Verified, Rejected, and All.

The approval RPC requires a current Nurse Health Clearance for applicable Incoming 1st Year Student records whose official `gender_sex` is explicitly `Female`. The current Nurse workspace is `/admin/clearances/health`; Registrar/Admin review remains at `/admin/enrollments` and shows missing, legacy, rejected, invalidated, or unavailable Nurse evidence as an approval block. Removing an official assignment blocks future signatures but does not invalidate or erase historical signatures because each signature snapshots the signer name, profile, role, and timestamp. Exact clearance order, whether every non-health official is mandatory, payment/zero-balance rules, and multi-role operational policy remain client decisions.
