...
- **Server-Only Email Abstraction**: Provides a pluggable `EmailAdapter` for one-time setup links and enrollment approval/rejection decision notifications (Resend when explicitly configured, Mock adapter for preview). Live delivery is disabled by default.
- **One-Time Setup Link and Decision Notice**: The setup path can generate a recovery/setup link to set an account password without exposing raw secrets. After a successful Registrar decision, the server can send a status notice without including rejection remarks. Neither path is generated-password delivery, and neither is used for live delivery unless PKM configures it.
