-- Add the workbook's existing program catalog before replaying its offerings.
-- The catalog values match the repository's existing SQLite development seed.
-- Existing rows, including the current BSAIS row, are preserved.
...
-- The original migration ran before these catalog rows existed in some
-- environments. Replay its idempotent workbook-derived inserts now.
