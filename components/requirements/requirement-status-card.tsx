...
export function RequirementStatusCard({
  ...
  return (
    <div className="rounded-lg border border-slateui-border bg-slateui-surface p-4 shadow-sm space-y-3">
      ...
      {applicability === "NOT_APPLICABLE" ? <p className="text-sm leading-6 text-slateui-secondary">Special Health Record form is not required for this student. Standard Nurse Health Clearance signature applies.</p> : null}
      {!unavailable ? (
        <div className="rounded-md border border-slateui-border bg-white p-3 text-sm text-slateui-secondary">
          ...
          <p className="mt-1">
            Evidence: <span className="font-semibold text-slateui-text">
              {nurseSignatureStatus === "SIGNED" ? "CURRENT SIGNATURE" : nurseSignatureStatus === "UNAVAILABLE" ? "UNAVAILABLE" : nurseSignatureStatus === "INVALIDATED" ? "INVALIDATED" : "SIGNATURE MISSING"}
            </span>
          </p>
